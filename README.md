...
(2) 지하철역 별 수요 공급 불균형 여부 분류
  ...
  - Data Analysis
    ...
    - 단순 반경과 도넛형 반경의 역 별 수요 공급 불균형 여부 비교
...
(2) 지하철역 별 수요 공급 불균형 여부 분류
  - 단순 반경보다 도넛형 반경에 따른 따릉이 잠재 수요 인원 집계 방식이 적합하다는 것을 확인
    - 지하철역들이 밀집해 있는 지역의 경우, 역에서 최종 목적지로 이동 시 도보 이동할 가능성이 높음
    - 역 주변 유동인구와 따릉이 수요량이 비례하지 않는다는 분석 결과 도출
      - ex. 을지로입구역: 역 주변 종사자 수가 많은 것에 비해 따릉이 적정 거치대 수 적음
...
## Review
  - 개선 및 발전방향
    ...
    - 각 지하철역 반경 내 정확한 잠재 수요 인원 추출 방법 고려
      - 100m 그리드 데이터를 활용한 현 상황에서 정확한 반경 내 인원 집계 불가
